...
# 2. List of species to compare ----

list_species <- c("Ostorhinchus apogonoides", "Epinephelus merra")

# 3. Make a function to get fitted values for each species ----

vbgf_bayesian <- function(data){
...
# 4. Get fitted values for back-calculated data ---- 

# 4.1 Extract the data --

data_backcalc <- data_complete %>%
...
# 4.2 Apply the function --

data_backcalc_vbgf <- data_backcalc %>% 
  group_by(Species, Type) %>% 
  do(vbgf_bayesian(data = .))

# 5. Get fitted values for raw data ---- 
...
# 5.2 Apply the function --

data_raw_vbgf <- data_raw %>% 
  group_by(Species, Type) %>% 
  do(vbgf_bayesian(data = .))
...
data_combined <- bind_rows(data_raw, data_backcalc)

# 6.2 Combine VBGF data --

data_combined_vbgf <- bind_rows(data_backcalc_vbgf, data_raw_vbgf)

# 6.3 Make the plot --

ggplot() +
  stat_lineribbon(data = data_combined_vbgf, aes(x = Agei, y = .value)) +
  scale_fill_brewer() +
  geom_point(data = data_combined, aes(x = Agei, y = Li_sp_m)) +
