...
# 5. Bayesian back calculation

# load stan model and load function
...
bc_results1 <- bc_results[!is.na(bc_results)] %>% lapply(function(x){x$lengths})%>% plyr::ldply() %>% mutate(agei = age) %>% dplyr::select(-age)

# merge
data_complete <- left_join(data_complete, bc_results1)

# 5. Make the back-calculation ----

# 5.1 Back-calculation for each species --

data_complete <- ddply(data_complete, .(species), function(x) backcalc(x))

data_complete$l_diff <- abs(data_complete$l_m - data_complete$li) # >> in some cases quite different....

# 5.2 Replacement of Li at age 0 by the value of L0p --
...
data_complete <- data_complete %>% 
  select(family, genus, species, id, agei, radi, agecap, radcap, lencap, l0p, r0p, li, biomass, location, observer) %>% 
  rename("Family" = "family",
         "Genus" = "genus",
         "Species" = "species",
         "ID" = "id",
...
write.csv(data_complete, "./../data/size_at_age_coral_reef_fishes_data.csv", row.names = FALSE)
