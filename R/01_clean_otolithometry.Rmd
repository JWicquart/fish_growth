...
# 2. Make the back-calculation with Bayesian method ----

# 2.1 Back-calculation for each species --
...
# 2.2 Merge with the data --

data_complete_sp <- left_join(data_complete, bc_results1)
...
# 1. Make the back-calculation with frequentist method ----

# 1.1 Back-calculation for each species --

data_complete_sp <- ddply(data_complete_sp, .(species), function(x) backcalc(x))

# 1.2 Replacement of Li at age 0 by the value of L0p --

data_complete_sp$li[data_complete_sp$agei == 0] <- data_complete_sp$l0p[data_complete_sp$agei == 0]
...
# 1. Re order variables ----

data_complete_sp <- data_complete_sp %>% 
  select(family, genus, species, id, agei, radi, agecap, radcap, lencap, l0p, r0p, l_m, li, biomass, location, observer) %>% 
  rename("Family" = "family",
         "Genus" = "genus",
         "Species" = "species",
         "ID" = "id",
...
write.csv(data_complete_sp, 
          "./../data/size_at_age_coral_reef_fishes_data_by-species.csv", 
          row.names = FALSE)
...
# 2.1 Back-calculation for each species and each location --
...
data_complete_sploc <- left_join(data_complete, bc_results1)

# 2.3 Add R0p --

data_complete_sploc <- data_complete_sploc %>% 
  filter(agei == "0") %>% 
  dplyr::group_by(species, location) %>% 
  summarize(r0p = mean(radi, na.rm = TRUE)) %>% 
  merge(., data_complete_sploc, all.y = TRUE)
...
data_complete_sploc <- data_complete_sploc %>% 
  select(family, genus, species, id, agei, radi, agecap, radcap, lencap, l0p, r0p, l_m, l_sd, l_q1, l_q3, biomass, location, observer) %>% 
  rename("Family" = "family",
         "Genus" = "genus",
         "Species" = "species",
         "ID" = "id",
...
write.csv(data_complete_sploc, 
          "./../data/size_at_age_coral_reef_fishes_data_by-species-and-location.csv", 
          row.names = FALSE)
