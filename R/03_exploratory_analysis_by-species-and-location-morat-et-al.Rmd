...
data_complete <- read.csv("./../data/size_at_age_coral_reef_fishes_data_by-species-and-location.csv")
...
# 1. Total nb individual by species ----
...
# 2. Nb individual by species for which Li != NA ----
...
ggplot(data_complete, aes(x = Agei, y = Li_m)) +
  geom_point(color = col_color_graph, fill = col_fill_graph, size = 1, shape = 21)+
  theme(strip.text.x = element_text(size = 8, face = "italic"),
        strip.background = element_rect(colour = "black", fill = col_facet, size = 1),
        plot.title = element_text(colour = col_color_graph),
...
data_complete %>% 
  ...
  plot_ly(., x = ~Agei, y = ~Li_m, text = ~paste("ID:", ID))
