...
data_raw_curve <- read.csv("./../data/03_raw_vbgf_fitted.csv") %>% 
  filter(Species %in% species_list) %>% 
  mutate(Type = "Raw")
...
data_backcalc_curve <- read.csv("./../data/03_back-calculated_vbgf_fitted.csv") %>% 
  filter(Species %in% species_list,
         !(Species == "Abudefduf sexfasciatus" & Location == "Moorea"),
         !(Species == "Acanthurus triostegus" & Location == "Marquesas"),
         !(Species == "Myripristis berndti" & Location == "Gambiers"),
...
ggplot() +
  ...
  geom_line(data = data_raw_curve, aes(x = Agei, y = Estimate, color = Type)) +
  ...
  geom_line(data = data_backcalc_curve, aes(x = age, y = ypred_m, color = Type)) +
...
UU <- vbgf_literature %>% 
  ...
  mutate(Estimate = as.numeric(Estimate),
         Type = "Literature") %>% 
  bind_rows(., read.csv("./../data/03_back-calculated_vbgf_predictions.csv") %>% 
...
ggplot(data = UU, aes(x = reorder(Species, desc(Species)), y = Estimate, color = Type)) +
  geom_point() +
